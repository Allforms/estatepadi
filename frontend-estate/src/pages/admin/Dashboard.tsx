...
const AdminDashboard: React.FC = () => {
  ...
  const statCards = [
    ...
    {
      title: "Estate Dues",
      value: Array.isArray(stats.estate_dues) ? stats.estate_dues.length : 0,
      ...
      <div className="space-y-6 sm:space-y-8 pb-24 sm:pb-6">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-3 sm:space-y-0 sm:space-x-3">
              ...
              <div className="flex-1">
                ...
                <p className="opacity-90 text-sm sm:text-base">
                  Some features are restricted. Renew your subscription to
                  unlock all features.
                </p>
              </div>
              <a
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
          {statCards.map((stat, i) => {
            const cardContent = (
              <div
                className={`${stat.bgColor} ${stat.borderColor} border rounded-lg sm:rounded-xl shadow-sm p-4 sm:p-6 hover:shadow-md transition-all ${stat.link && user?.subscription_active ? "cursor-pointer active:scale-95" : ""}`}
              >
                    ...
                    <div className="flex items-center space-x-2 sm:space-x-3 mb-2 sm:mb-3">
                      <div className="p-1.5 sm:p-2 bg-white rounded-lg shadow-sm">
                        {React.cloneElement(stat.icon as React.ReactElement, {
                          size: 20,
                          className: `sm:w-6 sm:h-6 ${(stat.icon as React.ReactElement).props.className}`,
                        })}
                      </div>
                      {stat.change && (
                        <span
                          className={`text-xs px-2 py-0.5 sm:py-1 rounded-full font-medium ${
          ...
          <div className="bg-white rounded-lg sm:rounded-xl shadow-sm border border-red-200 p-4 sm:p-6">
              ...
              <div className="flex items-center space-x-2 sm:space-x-3">
                ...
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-gray-500">
                    Latest alerts from your estate
                  </p>
                </div>
              </div>
            ...
            <div className="space-y-3 mt-4">
              {stats.recent_alerts.slice(0, 3).map((alert: any) => (
                <div
                  key={alert.id}
                  className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 sm:p-4 bg-red-50 border border-red-200 rounded-lg hover:bg-red-100 transition-colors space-y-2 sm:space-y-0"
                >
                  <div className="flex items-start space-x-2 sm:space-x-3">
                    <div className="p-1.5 sm:p-2 bg-red-200 rounded-full flex-shrink-0">
                      <AlertTriangleIcon
                        size={16}
                        className="sm:w-5 sm:h-5 text-red-700"
                      />
                    </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 space-y-3 sm:space-y-0">
              <div className="flex items-center space-x-2 sm:space-x-3">
                ...
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-gray-500">
                    Overview of registered staff members
                  </p>
                </div>
              </div>
              {user?.subscription_active && (
              ...
              <div className="bg-gradient-to-br from-green-50 to-green-100 border border-green-200 rounded-lg p-3 sm:p-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="p-2 sm:p-3 bg-green-200 rounded-full">
                    <CheckCircleIcon
                      size={16}
                      className="sm:w-5 sm:h-5 text-green-700"
                    />
                  </div>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            ...
            >
              <div className="flex items-center space-x-2 sm:space-x-3">
                <UsersIcon
                  size={18}
                  className={`sm:w-5 sm:h-5 ${user?.subscription_active ? "text-blue-600" : "text-gray-400"}`}
                />
                <span
                  className={`font-medium text-sm sm:text-base ${user?.subscription_active ? "text-blue-800" : "text-gray-400"}`}
                >
              ...
              {stats.pending_residents > 0 && user?.subscription_active && (
                <div className="mt-2 text-xs text-blue-600">
                  {stats.pending_residents} pending approval
                  {stats.pending_residents > 1 ? "s" : ""}
                </div>
              )}
            </Link>
            ...
            >
              <div className="flex items-center space-x-2 sm:space-x-3">
                <CreditCardIcon
                  size={18}
                  className={`sm:w-5 sm:h-5 ${user?.subscription_active ? "text-green-600" : "text-gray-400"}`}
                />
                <span
                  className={`font-medium text-sm sm:text-base ${user?.subscription_active ? "text-green-800" : "text-gray-400"}`}
                >
              ...
              {stats.pending_payments > 0 && user?.subscription_active && (
                <div className="mt-2 text-xs text-green-600">
                  {stats.pending_payments} pending payment
                  {stats.pending_payments > 1 ? "s" : ""}
                </div>
              )}
            </Link>
            ...
            >
              <div className="flex items-center space-x-2 sm:space-x-3">
                <MegaphoneIcon
                  size={18}
                  className={`sm:w-5 sm:h-5 ${user?.subscription_active ? "text-purple-600" : "text-gray-400"}`}
                />
                <span
                  className={`font-medium text-sm sm:text-base ${user?.subscription_active ? "text-purple-800" : "text-gray-400"}`}
                >
                  Add New Due Notice
                </span>
              </div>
            ...
            >
              <div className="flex items-center space-x-2 sm:space-x-3">
                <BellIcon
                  size={18}
                  className={`sm:w-5 sm:h-5 ${user?.subscription_active ? "text-red-600" : "text-gray-400"}`}
                />
                <span
                  className={`font-medium text-sm sm:text-base ${user?.subscription_active ? "text-red-800" : "text-gray-400"}`}
                >
              ...
              {stats.total_alerts &&
                ...
                user?.subscription_active && (
                  <div className="mt-2 text-xs text-red-600">
                    {stats.total_alerts} total alert
                    {stats.total_alerts > 1 ? "s" : ""}
                  </div>
                )}
            </Link>
          </div>
