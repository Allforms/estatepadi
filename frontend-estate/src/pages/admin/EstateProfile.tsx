...
const EstateProfile: React.FC = () => {
  ...
  useEffect(() => {
    ...
    api
      .get(`/api/estates/${estateId}/leadership/`)
      .then((res) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      try {
        ...
        alert(
          "Estate updated successfully! Note: Bank account changes may not be saved yet.",
        );
      }
  ...
  const removeBankAccount = async (accountId: string | number) => {
    if (typeof accountId === "string" && accountId.startsWith("new-")) {
      ...
      try {
        ...
        setBankAccounts(bankAccounts.filter((acc) => acc.id !== accountId));
        alert(
          "Bank account removed from list. Changes will be saved when you save the estate.",
        );
      }
    }
  };
  ...
  const saveBankAccounts = async () => {
    try {
      ...
      const newAccounts = bankAccounts.filter(
        (acc) =>
          acc.isNew && acc.account_number && acc.account_name && acc.bank_name,
      );
      ...
      try {
        ...
        for (const account of existingAccounts) {
          await api.put(
            `/api/estates/${estateId}/bank-accounts/${account.id}/`,
            {
              account_number: account.account_number,
              account_name: account.account_name,
              bank_name: account.bank_name,
        ...
        const allAccounts = bankAccounts
          .filter(
            (acc) => acc.account_number && acc.account_name && acc.bank_name,
          )
          .map((acc) => ({
        ...
        try {
          ...
          throw new Error(
            "Bank account management is not yet implemented in the backend. Please contact your administrator.",
          );
        }
      }
            ...
            <div className="flex space-x-3">
              {isEditing ? (
                ...
                <>
                  ...
                  <button
                    onClick={() =>
                      (window.location.href = `/admin/estates/${estateId}/leadership`)
                    }
          ...
          {isEditing && (
            <div className="bg-blue-50 border border-blue-200 rounded-md p-3">
              <p className="text-sm text-blue-800">
                Click the camera icon on the logo to change it. Supported
